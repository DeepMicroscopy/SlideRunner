    ...
    def queueWorker(self):
        ...
        while not quitSignal:
            ...
            if (len(job.openedDatabase.annotations) != last_annotations_count) or int(job.configuration[1]) != self.headmap_resolution:
                ...
                x_steps = range(0, 35000 - 2 * self.headmap_resolution, int(self.headmap_resolution / 2))
                y_steps = range(0, 35000 - 2 * self.headmap_resolution, int(self.headmap_resolution / 2))
                ...
                self.overlay = cv2.applyColorMap(gt_image, cv2.COLORMAP_JET)

                # Mask overlay
                self.overlay[np.array(gt_image == 0)[:, :, [0, 0, 0]]] = [255]
