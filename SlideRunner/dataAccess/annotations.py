...
class annotation():
      ...
      def draw(self, image: np.ndarray, leftUpper: tuple, zoomLevel: float, thickness: int = 1):
            return
      ...
      def getColor(self):
          return COLORS_CLASSES[self.agreedLabel()]
...
class rectangularAnnotation(annotation):
      ...
      def draw(self, image: np.ndarray, leftUpper: tuple, zoomLevel: float, thickness: int):
            ...
            image = cv2.rectangle(image, thickness=thickness, pt1=(xpos1,ypos1), pt2=(xpos2,ypos2),color=self.getColor(), lineType=cv2.LINE_AA)
...
class polygonAnnotation(annotation):
    ...
    def draw(self, image: np.ndarray, leftUpper: tuple, zoomLevel: float, thickness: int):
        ...
        for listIdx in range(self.coordinates.shape[0]-1):
            ...
            cv2.line(img=image, pt1=anno, pt2=slideToScreen(self.coordinates[listIdx+1]), thickness=2, color=self.getColor(), lineType=cv2.LINE_AA)       
            ...
            cv2.rectangle(img=image, pt1=(pt1_rect), pt2=(pt2_rect), color=self.getColor(), thickness=1)
        listIdx+=1
        anno = slideToScreen(self.coordinates[listIdx])
        pt1_rect = (max(0,anno[0]-markersize),
                    max(0,anno[1]-markersize))
        ...
        cv2.line(img=image, pt1=anno, pt2=slideToScreen(self.coordinates[0]), thickness=2, color=self.getColor(), lineType=cv2.LINE_AA)       
...
class circleAnnotation(annotation):
      ...
      def draw(self, image: np.ndarray, leftUpper: tuple, zoomLevel: float, thickness: int):
            ...
            if (radius>=0):
                  image = cv2.circle(image, thickness=thickness, center=(xpos1,ypos1), radius=radius,color=self.getColor(), lineType=cv2.LINE_AA)
